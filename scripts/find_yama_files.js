...
async function findYamaFiles() {
    console.log('Searching for any file containing "yama" in all folders...');
    ...
    for (const folder of folders) {
        ...
        if (data) {
            const found = data.filter(f => f.name.toLowerCase().includes('yama'));
            if (found.length > 0) {
