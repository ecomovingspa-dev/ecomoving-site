...
async function listFiles() {
    try {
        const { data, error } = await supabase.storage.from('imagenes-marketing').list('', { recursive: true });
        if (error) {
            console.error('Error listing files in imagenes-marketing:', error);
        } else {
            console.log('ALL Files in imagenes-marketing:', JSON.stringify(data, null, 2));
        }
    } catch (e) {
